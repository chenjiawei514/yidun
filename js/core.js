...
var i = function (e, t) {
    ...
    u = function (e) {
        var t = e.length;
        if (null == e || t < 0) return String('');
        for (var n = [], i = 0; i < t; i++) n.push(s(e[i]));
        return n.join('')
    },
    f = function (e) {
        if (null == e || 0 == e.length) return [];
        for (var t = String(e), n = [], i = t.length / 2, r = 0, o = 0; o < i; o++) {
    ...
    $ = function (e) {
        ...
        if (e.length >= k) return h(e, 0, k);
        for (var n = 0; n < k; n++) t[n] = e[n % e.length];
        return t
    },
    I = function (e) {
    ...
    x = function (e) {
        if (null == e || e.length % C != 0) throw new Error('1005');
        for (var t = [], n = 0, i = e.length / C, r = 0; r < i; r++) {
            t[r] = [];
            for (var o = 0; o < C; o++) t[r][o] = e[n++]
        }
        return t
    },
    A = function (e) {
...
function encrpty_tracedata(token, traceData) {
    // 先对每个轨迹进行加密
    encrpty_tracedata_list = [];
    for (var i = 0; i < traceData.length; i++) {
        var tmp = n(token, traceData[i].join(','));
        console.log(traceData[i], tmp)
        encrpty_tracedata_list.push(tmp);
    }
    var t = sample(encrpty_tracedata_list, 50),
        r = B(n(token, parseInt(traceData[traceData.length - 1][0] - 11, 10) / 320 * 100 + ''));
